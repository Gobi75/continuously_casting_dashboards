...
class SwitchEntityChecker:
    ...
    def __init__(self, hass: HomeAssistant, config: dict):
        ...
        self.switch_entity_state = config.get(
            CONF_SWITCH_ENTITY_STATE
        )  # Get global custom state
        ...
        if self.switch_entity_id:
            ...
            if state is None:
                _LOGGER.error(
                    f"CRITICAL: Global switch entity {self.switch_entity_id} NOT FOUND in Home Assistant!"
                )
            else:
                _LOGGER.debug(f"GLOBAL SWITCH ENTITY CURRENT STATE: {state.state}")
        else:
            _LOGGER.debug(
                "NO GLOBAL SWITCH ENTITY CONFIGURED - Casting will be controlled per device"
            )
    ...
    async def async_check_switch_entity(self, device_name=None, device_config=None):
        ...
        if device_name and device_config and "switch_entity_id" in device_config:
            ...
            device_state = device_config.get(
                "switch_entity_state"
            )  # Get device-specific custom state

            _LOGGER.debug(
                f"Checking device-specific switch entity for {device_name}: {device_switch}"
            )
            if device_state:
                _LOGGER.debug(
                    f"Device-specific custom state for {device_name}: {device_state}"
                )
            ...
            if device_switch:
                ...
                if state is None:
                    _LOGGER.warning(
                        f"Device switch entity {device_switch} not found for {device_name}"
                    )
                else:
                    ...
                    if device_state:
                        ...
                        is_enabled = state.state.lower() in (
                            "on",
                            "true",
                            "home",
                            "open",
                    ...
                    _LOGGER.debug(
                        f"Device {device_name} - Switch Entity: {device_switch} state = {state.state}, casting enabled: {is_enabled}"
                    )
                    return is_enabled
        ...
        if state is None:
            _LOGGER.debug(
                f"Global switch entity {self.switch_entity_id} not found in Home Assistant states"
            )
        ...
        _LOGGER.debug(
            f"Global Switch Entity: {self.switch_entity_id} state = {state.state}, casting enabled: {is_enabled}"
        )
        return is_enabled
